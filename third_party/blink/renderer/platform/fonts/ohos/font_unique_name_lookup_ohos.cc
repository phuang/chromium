  ...
  if (RuntimeEnabledFeatures::AndroidDownloadableFontsMatchingEnabled()) {
    ...
    if (android_font_lookup_service_) {
      ...
      android_font_lookup_service_->FetchAllFontFiles(
          WTF::BindOnce(&FontUniqueNameLookupOHOS::FontsPrefetched,
                        WTF::Unretained(this)));
    }
  }
...
sk_sp<SkTypeface> FontUniqueNameLookupOHOS::MatchUniqueNameFromFirmwareFonts(
    const String& font_unique_name) {
...
bool FontUniqueNameLookupOHOS::RequestedNameInQueryableFonts(
    const String& font_unique_name) {
  if (!queryable_fonts_) {
...
sk_sp<SkTypeface>
FontUniqueNameLookupOHOS::MatchUniqueNameFromDownloadableFonts(
    const String& font_unique_name) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void FontUniqueNameLookupOHOS::FontsPrefetched(
