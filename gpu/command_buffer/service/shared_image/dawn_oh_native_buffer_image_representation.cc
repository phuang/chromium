...
DawnOHNativeBufferImageRepresentation::DawnOHNativeBufferImageRepresentation(
...
wgpu::Texture DawnOHNativeBufferImageRepresentation::BeginAccess(
  ...
  if (access_mode == AccessMode::kWrite) {
    ...
    if (!ohnb_backing()->BeginRead(this, &sync_fd)) {
      return nullptr;
    }
  }
  ...
  if (!shared_texture_memory_) {
    wgpu::SharedTextureMemoryOHNativeBufferDescriptor stm_desc = {};
    stm_desc.handle = buffer_.buffer();

    wgpu::SharedTextureMemoryDescriptor desc = {};
    desc.nextInChain = &stm_desc;
    shared_texture_memory_ = device_.ImportSharedTextureMemory(&desc);
  }

  if (!shared_texture_memory_) {
    LOG(ERROR) << "Failed to create the SharedTextureMemory";
    return {};
  }

  texture_ = shared_texture_memory_.CreateTexture(&texture_descriptor);
  if (!texture_) {
    LOG(ERROR) << "Failed to create the Texture from the SharedTextureMemory";
    return {};
  }
...
void DawnOHNativeBufferImageRepresentation::EndAccess() {
  ...
  absl::Cleanup on_exit = [this, &end_access_sync_fd]() {
    if (access_mode_ == AccessMode::kWrite) {
      ohnb_backing()->EndWrite(std::move(end_access_sync_fd));
    } else {
      ohnb_backing()->EndRead(this, std::move(end_access_sync_fd));
    }
    texture_.Destroy();
    texture_ = nullptr;
