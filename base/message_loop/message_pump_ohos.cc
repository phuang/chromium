...
NO_INSTRUMENT_STACK_ALIGN void NonDelayedLoopCallback(uv_poll_t* handle,
                                                      int status,
                                                      int events) {
  ...
  DCHECK(events & UV_READABLE);
  MessagePumpOHOS* pump = reinterpret_cast<MessagePumpOHOS*>(handle->data);
  pump->OnNonDelayedLoopCallback();
  return;
}

NO_INSTRUMENT_STACK_ALIGN void DelayedLoopCallback(uv_poll_t* handle,
                                                   int status,
                                                   int events) {
  if (events & UV_DISCONNECT) {
    return;
...
napi_env g_env_ = nullptr;

}  // namespace

// static
void MessagePumpOHOS::SetNapiEnv(napi_env env) {
  DCHECK(!g_env_);
  g_env_ = env;
}
...
MessagePumpOHOS::MessagePumpOHOS() {
  ...
  napi_status status = napi_get_uv_event_loop(g_env_, &loop);
  DCHECK_EQ(status, napi_ok);
  DCHECK(loop);
  ...
  delayed_fd_ = timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK | TFD_CLOEXEC);
  CHECK_NE(delayed_fd_, -1);

  int ret = uv_poll_init(loop_.get(), &non_delayed_poll_, non_delayed_fd_);
  DCHECK_EQ(ret, 0);
  non_delayed_poll_.data = this;

  ret = uv_poll_init(loop_.get(), &delayed_poll_, delayed_fd_);
  DCHECK_EQ(ret, 0);
  delayed_poll_.data = this;

  ret = uv_poll_start(&non_delayed_poll_, UV_READABLE, NonDelayedLoopCallback);
  DCHECK_EQ(ret, 0);

  ret = uv_poll_start(&delayed_poll_, UV_READABLE, DelayedLoopCallback);
  DCHECK_EQ(ret, 0);

  uv_async_send(&loop_->wq_async);
}

MessagePumpOHOS::~MessagePumpOHOS() {
  DCHECK_EQ(uv_default_loop(), loop_);

  uv_poll_stop(&non_delayed_poll_);
  uv_poll_stop(&delayed_poll_);
  close(non_delayed_fd_);
  close(delayed_fd_);
}

void MessagePumpOHOS::InitializeFeatures() {
  g_fast_to_sleep = base::FeatureList::IsEnabled(kPumpFastToSleepAndroid);
}

void MessagePumpOHOS::OnDelayedLoopCallback() {
  ...
  napi_status status = napi_is_exception_pending(g_env_, &is_pending);
  DCHECK_EQ(status, napi_ok);
  if (is_pending) {
    return;
  }
...
void MessagePumpOHOS::DoDelayedLoopWork() {
...
void MessagePumpOHOS::OnNonDelayedLoopCallback() {
  // There may be non-Chromium callbacks on the same ALooper which may have left
  // a pending exception set, and ALooper does not check for this between
  // callbacks. Check here, and if there's already an exception, just skip this
  // iteration without clearing the fd. If the exception ends up being non-fatal
  ...
  DoNonDelayedLoopWork(do_idle_work);
}

void MessagePumpOHOS::DoNonDelayedLoopWork(bool do_idle_work) {
  ...
  do {
    ...
    // to
    // // be handled. In some cases input events can remain undetected. Such
    // "input
...
void MessagePumpOHOS::Attach(Delegate* delegate) {
  DCHECK(!quit_);
...
void MessagePumpOHOS::ScheduleWorkInternal(bool do_idle_work) {
...
void MessagePumpOHOS::ScheduleDelayedWork(
...
void MessagePumpOHOS::QuitWhenIdle(base::OnceClosure callback) {
...
MessagePump::Delegate* MessagePumpOHOS::SetDelegate(Delegate* delegate) {
  return std::exchange(delegate_, delegate);
}

bool MessagePumpOHOS::SetQuit(bool quit) {
