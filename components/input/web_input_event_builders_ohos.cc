...
#include "components/input/web_input_event_builders_ohos.h"
...
//   // Currently only Shift, CapsLock are used, add other modifiers if
//   required. if (web_modifier & WebInputEvent::kShiftKey)
...
//     // According to spec |kAllowedModifiers| should be Shift and AltGr,
//     however
//     // Android doesn't have AltGr key and ImeAdapter::getModifiers won't pass
//     it
//     // either.
//     // According to discussion we want to honor CapsLock and possibly NumLock
//     as
...
WebKeyboardEvent WebKeyboardEventBuilder::Build(const void* ohos_key_event,
...
WebMouseEvent WebMouseEventBuilder::Build(const void* motion_event,
                                          WebInputEvent::Type type,
                                          int click_count,
                                          int action_button) {
  ...
  // result.SetPositionInScreen(motion_event.GetRawX(0),
  // motion_event.GetRawY(0));

  // result.click_count = click_count;

  // int button = action_button;
  // // For events other than MouseDown/Up, action_button is not defined. So we
  // are
  // // determining |button| value from |modifiers| as is done in other
  // platforms. if (type != WebInputEvent::Type::kMouseDown &&
...
WebMouseWheelEvent WebMouseWheelEventBuilder::Build(const void* motion_event) {
  // WebMouseWheelEvent result(WebInputEvent::Type::kMouseWheel,
  //                           WebInputEvent::kNoModifiers,
  //                           motion_event.GetEventTime());
  // result.SetPositionInWidget(motion_event.GetX(0), motion_event.GetY(0));
  // result.SetPositionInScreen(motion_event.GetRawX(0),
  // motion_event.GetRawY(0)); result.button = WebMouseEvent::Button::kNoButton;
